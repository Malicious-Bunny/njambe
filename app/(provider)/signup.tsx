...
import { EyeClosed, Eye, Google, Check, NavArrowLeft } from 'iconoir-react-native';
...
export default function ProviderSignupScreen() {
  ...
  // Handle back - go to main index, not onboarding
  const handleBack = () => {
    router.replace('/');
  };
  ...
  const handleSignup = () => {
    ...
    // Success - navigate to provider home
  ...
  return (
    <SafeAreaView className="flex-1 bg-background" edges={['top']}>
      <KeyboardAvoidingView
        ...
        className="flex-1"
      >
        {/* Custom Header with back to home */}
        <View className="flex-row items-center px-4 py-3">
          <Pressable
            onPress={handleBack}
            className="flex-row items-center py-2 pr-4"
          >
            <NavArrowLeft width={24} height={24} color={textColor} />
          </Pressable>
          <View className="flex-1 items-center pr-10">
            <Text className="text-lg font-semibold text-foreground">Join njambe!</Text>
          </View>
        </View>
